...
import { UserPlan, PlanDetails } from "./Subscription";
...
function SubscriptionView({
  ...
  const message = cancel_at_period_end ? (
    ...
    <>
      Your card will be automatically charged <strong>${amount || "0"}</strong> on{" "}
      <strong>{datetime.getFriendlyDate(current_period_end)}</strong>
    </>
  );
  ...
  return (
    <Card padding="15px">
      <VStack align="flex-start">
        ...
        <Badge colorScheme={planColorScheme}>{currentPlan}</Badge>
        <Text color="gray.500">{message}</Text>
        <HStack paddingTop="5px">
          <Button size="sm" onClick={handleManageSubscription} isLoading={redirecting}>
            Manage Subscription
          </Button>
        </HStack>
      </VStack>
